...
contract('Bridge - [deposit - Native coin]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        OriginNativeHandlerInstance = await NativeHandlerContract.new(BridgeInstance.address);
        DestinationNativeHandlerInstance = await NativeHandlerContract.new(BridgeInstance2.address);

        await Promise.all([
            BridgeInstance.adminSetResource(OriginNativeHandlerInstance.address, resourceID, zeroAddress),
            BridgeInstance2.adminSetResource(DestinationNativeHandlerInstance.address, resourceID, burnableContractAddresses),
            BridgeInstance2.adminSetBurnable(DestinationNativeHandlerInstance.address, burnableContractAddresses),
